...
description: AionUi Tutorial — A free open-source desktop office agent supporting various AI agents like Gemini CLI, Claude Code, Codex, and more. Integrates with FlyReq API to enable multi-model workflows.
---
...
Provider Type: Types supported by FlyReqAPI
API Key: Obtained from FlyReqAPI
API Address: FlyReqAPI site address (e.g., `https://<your-flyreqapi-domain>/v1`)

Please use your self-deployed FlyReq API, or ensure that the service provider offers FlyReq API services with legitimate upstream authorization and compliance obligations. Do not connect API addresses or keys from unknown sources to a production environment.

### Configuration Steps

1.  **Copy API key in FlyReqAPI**
...
3.  **Add New Provider**
    - Click "Add Model"
    - Select FlyReqAPI
    ![Add FlyReqAPI Provider](/assets/aionui/flyreqapi_provider.png)

4.  **Configure API Information**
    - API Address: Enter your FlyReqAPI site address (format: `https://<your-flyreqapi-domain>/v1`)
    - API Key: Paste the API Key copied from the FlyReqAPI console

5.  **Add Model**
    - Select the model to add from the dropdown
    - The model name should match the model name configured in FlyReqAPI
    - Select the appropriate request protocol

6.  **Start Using**
    - Return to the chat page
    - Select the configured FlyReqAPI model to start a conversation
