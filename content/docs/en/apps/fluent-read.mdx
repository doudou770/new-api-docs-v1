...
description: FluentRead configuration guide — open-source browser translation plugin with 20+ engines and AI model support. Connect to FlyReq API for immersive bilingual reading.
---

<Callout type="info" title="Chat Settings Options">
  In the System Settings -> Chat Settings of the FlyReq API console, you can add
...
### Import Configuration from FlyReq API Console (Recommended)

After the FluentRead plugin is installed in your browser, opening the FlyReq API console -> Token Management page will display a prompt to add FluentRead.
...
After confirming the import, the FlyReq API configuration in FluentRead will be enabled.
...
| ------------------- | --------------------------------------- |
| Translation Service | FlyReqAPI                                  |
| Access Token        | FlyReqAPI Key                              |
| FlyReqAPI Endpoint     | FlyReqAPI Deployment Address (without /v1) |
